...
\begin{itemize}
  ...
  \item In these new parallel interfaces, the execution policy parameter can be used with:
     \begin{itemize}
        \item Iterator based algorithms with updated interfaces.
        \item Range based algorithms with updated interfaces.
    \end{itemize}
...
\begin{frame}[t,fragile]{New parallel iterator/range interfaces require random access}
\begin{itemize}
    \item These new iterator based parallel algorithms require random access iterators.
      \begin{itemize}
        \item Both input and output iterators must be random access.
      \end{itemize}

    \item These new range based parallel algorithms require random access ranges.
      \begin{itemize}
        \item Both input and output ranges must be random access.
      \end{itemize}

    \mode<presentation>{\vfill\pause}
    \item \textmark{Consequences for these interfaces}:
      \begin{itemize}
        \item They cannot be used with input, forward, or bidirectional iterators/ranges.
        \item They cannot be used with output iterators/ranges that are not random access.
        \item They cannot be used with some views (e.g., \cppcode{std::views::filter}).
      \end{itemize}
    ...
    \item \textmark{Why?}:
      \begin{itemize}
        ...
        \item For these interfaces, random access allows for efficient partitioning and load balancing.
      \end{itemize}
\end{itemize}
\end{frame}

\begin{frame}[t,fragile]{Parallel algorithms require sized ranges/sentinels}
\begin{itemize}
  \item New iterator based parallel algorithms require sized sentinels.
    \begin{itemize}
