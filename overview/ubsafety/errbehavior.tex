...
\begin{frame}[t,fragile]{Uninitialized variables}
	\begin{itemize}
		\item \textmark{Uninitialized variables}: Reading and uninitialized variable is undefined behavior.
		      \begin{lstlisting}
void f() {
  int count;
...
\begin{frame}[t,fragile]{C++26: Erroneous behavior}
	\begin{itemize}
      ...
      \begin{itemize}
        \item Well defined behavior that implementation is recommended to diagnose and report.
        \item Variable initialized to a well defined but implementation-defined value.
      \end{itemize}
\begin{lstlisting}
void f() {
  int count;
...
\end{lstlisting}
    ...
    \mode<presentation>{\vfill\pause}
    \item Escape attribute when non-initialization is intentional:
\begin{lstlisting}
void f() {
