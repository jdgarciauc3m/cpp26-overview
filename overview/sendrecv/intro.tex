...
int main() {
  ...
  std::scheduler auto sch = context.get_scheduler();

  std::sender auto begin = std::execution::just(42);
  std::sender auto multiply = std::execution::then(begin, [](int value) {
    return value * 2;
  });
  std::sender auto print = std::execution::then(multiply, [](int result) {
    std::cout << "Result: " << result << std::endl;
  });
...
int main() {
  ...
  std::sender auto work = std::execution::just(42)
